...
function checksession()
{
   ...
   if (isset($_COOKIE["loguserid"]) AND isset($_COOKIE["logsession"]))
      ...
      $userid=$db->conn->real_escape_string(trim($_COOKIE["loguserid"]));
      $session=$db->conn->real_escape_string(trim($_COOKIE["logsession"]));
      $result=$db->query("SELECT sessionId FROM sessions WHERE userId='$userid' AND sessionId='$session'");
      if ($result->num_rows==1)
         {
         ...
         $db->conn->commit();
         setcookie("loguserid","",false);
         setcookie("logsession","",false);
...
function checkbikeno($bikeNum)
{
   ...
   if (!$result->num_rows)
      {
      response('<h3>Le vélo '.$bikeNum.' n\'existe pas!</h3>',ERROR);
      }
}
...
function checkstandname($stand)
{
   ...
   if (!$result->num_rows)
      {
      response('<h3>'._('La station').' '.$stand.' '._('n\'existe pas').'!</h3>',ERROR);
      }
}
...
function sendConfirmationEmail($emailto)
{
   ...
   _('Vous avez été inscrit sur le service Cyclovis').' '.$systemname.".\n\n".
   _('Les règles d\'utilisation sont disponibles ici')."\n".$systemrules."\n\n".
   _('En cliquant sur le lien suivant, vous acceptez les termes d\'utilisation du service :')."\n".$systemURL."agree.php?key=".$userKey;
...
function confirmUser($userKey)
{
        ...
        if($result->num_rows==1)
                ...
                {
                        echo '<div class="alert alert-danger" role="alert">',_('Clé introuvable !'),'</div>';
                        return FALSE;
                }
        ...
        echo '<div class="alert alert-success" role="alert">',_('Votre compt a bien été créé. Bienvenue!'),'</div>';
...
function checktopofstack($standid)
{
   ...
   if (count($currentbikes))
      ...
      $result=$db->query("SELECT bikeNum FROM history WHERE action IN ('DEPOT','FORCEDEPOSER') AND parameter='$standid' AND bikeNum IN (".implode($currentbikes,",").") ORDER BY time DESC LIMIT 1");
      if ($result->num_rows)
         {
         $row=$result->fetch_assoc();
         return $row["bikeNum"];
   ...
   while($row=$result->fetch_assoc())
      ...
      if ($result2->num_rows)
         ...
         if ($time+($watches["longrental"]*3600)<=time())
            {
            $abusers.=" b".$bikenum." "._('par')." ".$username.",";
            $found=1;
            if ($notifyuser) sendSMS($userphone,_('Merci de retourner votre vélo ').$bikenum._(' à la station la plus proche ! Ignorer cet avertissement peut entrainer un banissement du service.'));
            }
         }
      }
   if ($found)
      ...
      notifyAdmins($watches["longrental"]."+ "._('heure de location').":".$abusers);
      }
   ...
   if ($cron) // called from cron
      ...
      while($row=$result->fetch_assoc())
         ...
         if ($result2->num_rows>=($userlimit+$watches["numbertoomany"]))
            {
            $abusers.=" ".$result2->num_rows." ("._('limite')." ".$userlimit.") "._('par')." ".$username.",";
            $found=1;
            }
         }
      }
      ...
      if ($result->num_rows>=($userlimit+$watches["numbertoomany"]))
         {
         $abusers.=" ".$result->num_rows." ("._('limite')." ".$userlimit.") "._('par')." ".$username.",";
         $found=1;
         }
      }
   if ($found)
      {
      $abusers=substr($abusers,0,strlen($abusers)-1);
      notifyAdmins(_('Limite dépassée')." ".$watches["timetoomany"]." "._('hs').":".$abusers);
      }
...
function changecreditendrental($bike,$userid)
{
   ...
   $result=$db->query("SELECT time FROM history WHERE bikeNum=$bike AND userId=$userid AND (action='RETIRE' OR action='FORCERETIRER') ORDER BY time DESC LIMIT 1");
   if ($result->num_rows==1)
      {
      $row=$result->fetch_assoc();
